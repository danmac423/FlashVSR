...
def generate_draft_block_mask(batch_size, nheads, seqlen, q_w, k_w, topk=10, local_attn_mask=None):
    ...
    local_attn_mask = local_attn_mask.masked_fill(local_attn_mask == False, -float("inf"))  # noqa: E712
    local_attn_mask = local_attn_mask.masked_fill(local_attn_mask == True, 0)  # noqa: E712
...
def generate_draft_block_mask_sage(
    ...
    D = q_w.shape[-1] // nheads
    q_heads = rearrange(torch.mean(q_w, dim=1), "s (h d) -> h s d", h=nheads)

    # Pool each KV block in two halves (instead of one) for a finer-grained draft.
    k_w_split = k_w.view(k_w.shape[0], 2, k_w.shape[1] // 2, k_w.shape[2])
    k_heads = rearrange(
        torch.mean(k_w_split, dim=2), "s two (h d) -> h (s two) d", h=nheads
    )  # shape: (h, s*2, d)
    ...
    scores = scores.masked_fill(~local_attn_mask, -float("inf"))
...
def flash_attention(
    ...
    if attention_mask is not None and mask_attn_mode == MaskAttentionMode.SPARSE_SAGE:
        if SPARSE_SAGE_ATTN_AVAILABLE:
            q = rearrange(q, "b s (n d) -> b n s d", n=num_heads)
            k = rearrange(k, "b s (n d) -> b n s d", n=num_heads)
            v = rearrange(v, "b s (n d) -> b n s d", n=num_heads)
            x = block_sparse_sage2_attn_cuda(
                q, k, v, mask_id=attention_mask.to(torch.int8), tensor_layout="HND"
            )
            x = rearrange(x, "b n s d -> b s (n d)", n=num_heads)
            return x
        _logger.warning(
            "SPARSE_SAGE attention requested but spas_sage_attn is not installed. "
            "Falling back to dense attention."
        )
        attention_mask = None

    if attention_mask is not None and mask_attn_mode == MaskAttentionMode.BLOCK_SPARSE:
        if BLOCK_SPARSE_ATTN_AVAILABLE:
            seqlen = q.shape[1]
            seqlen_kv = k.shape[1]
            ...
            return x
        _logger.warning(
            "BLOCK_SPARSE attention requested but block_sparse_attn is not installed. "
            "Falling back to dense attention."
        )
        attention_mask = None

    if attn_mode == AttentionMode.SAGE and SAGE_ATTN_AVAILABLE:
        q = rearrange(q, "b s (n d) -> b n s d", n=num_heads)
        k = rearrange(k, "b s (n d) -> b n s d", n=num_heads)
        v = rearrange(v, "b s (n d) -> b n s d", n=num_heads)
        x = sageattn(q, k, v)
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        x = rearrange(
            x, "b (block_n block_s) d -> (b block_n) (block_s) d", block_n=block_n, block_s=block_s
        )
        ...
        for name, param in state_dict.items():
            if name in rename_dict:
                ...
                if name_ in rename_dict:
                    ...
                    name_ = ".".join(
                        name_.split(".")[:1] + [name.split(".")[1]] + name_.split(".")[2:]
                    )
