...
def load_state_dict_from_folder(file_path, torch_dtype=None):
    ...
    for file_name in os.listdir(file_path):
        if "." in file_name and file_name.split(".")[-1] in [
            ...
            state_dict.update(
                load_state_dict(os.path.join(file_path, file_name), torch_dtype=torch_dtype)
            )
    return state_dict
    ...
    for key, value in state_dict.items():
        if isinstance(key, str):
            if isinstance(value, torch.Tensor):
                ...
                keys.append(
                    key + "|" + convert_state_dict_keys_to_single_str(value, with_shape=with_shape)
                )
    keys.sort()
    keys_str = ",".join(keys)
    return keys_str
...
class Buffer_LQ4x_Proj(nn.Module):
    def __init__(self, in_dim, out_dim, layer_num=30):
        ...
        self.conv2 = CausalConv3d(
            self.hidden_dim1, self.hidden_dim2, (4, 3, 3), stride=(2, 1, 1), padding=(1, 1, 1)
        )  # f -> f/2 h -> h w -> w
