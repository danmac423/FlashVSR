...
def apply_model_with_memblocks(model, x, parallel, show_progress_bar, mem=None):
    ...
    if parallel:
        ...
        for b in tqdm(model, desc="TCDecoder", leave=False, disable=not show_progress_bar):
        ...
        progress_bar = tqdm(range(T), desc="TCDecoder", leave=False, disable=not show_progress_bar)
        while work_queue:
...
class TAEW2_1DiffusersWrapper(nn.Module):
    def __init__(self, pretrained_path=None, channels=[256, 128, 64, 64]):
        ...
        self.config = DotDict(
            scaling_factor=1.0, latents_mean=torch.zeros(16), z_dim=16, latents_std=torch.ones(16)
        )
...
def build_tcdecoder(
    new_channels=[512, 256, 128, 128], device="cuda", dtype=torch.bfloat16, new_latent_channels=None
):
    """
    ...
    if new_latent_channels is not None:
        big = (
            TAEHV(checkpoint_path=None, channels=new_channels, latent_channels=new_latent_channels)
            .to(device)
            .to(dtype)
            .train()
        )
